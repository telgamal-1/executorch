...
/*
 * The constant_buffer path is deprecated from ExecuTorch 0.7 and has known
 * security issues; it is retained only for backwards compatibility with older
 * PTE files. Disable it by passing -DET_ENABLE_DEPRECATED_CONSTANT_BUFFER=0;
 * PTE files that rely on this path are rejected at load time.
 */
